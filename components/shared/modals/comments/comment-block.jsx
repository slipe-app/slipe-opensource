...
export default function CommentBlock({ id, user, content, likes, liked, date }) {
				...
				<div className='w-full flex gap-3 duration-200 ease-out items-center overflow-hidden'>
					...
					<div className='flex flex-col w-full overflow-hidden'>
						<div className='w-full flex gap-1'>
							<div className='whitespace-nowrap overflow-hidden max-w-fit text-ellipsis font-medium text-white'>{user?.nickname ? user?.nickname : user?.username}</div>
						</div>
						<span className='text-sm text-white/50'>{TimePassedFromDate(date)}</span>
					</div>
				</div>
				<Button data-liked={localLiked} className='rounded-full data-[liked=true]:bg-red-foreground hover:bg-red-foreground bg-white/[0.08] px-4 min-h-11 h-11 text-sm gap-2' onClick={likeComment}>
