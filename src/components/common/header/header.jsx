...
export default function Header() {
	...
	return (
		<header style={{ backgroundColor: colors.navigationBackground }} className='w-screen fixed flex gap-4 backdrop-blur-3xl z-10 p-4'>
			<a href='/search' style={{ backgroundColor: url == "/search" ? colors.text : colors.buttonInactiveBackground }} className='rounded-full duration-150 p-[0.625rem]'>
			...
			{StatesRender(url)}
			<a href='/notifs' style={{ backgroundColor: url == "/notifs" ? colors.text : colors.buttonInactiveBackground }} className='rounded-full duration-150 p-[0.625rem]'>
				<svg width='32' height='32' style={{ color: url == "/notifs" ? colors.background : colors.text }} viewBox='0 0 24 24' fill='currentColor' xmlns='http://www.w3.org/2000/svg'>
