		...
		<>
			{url !== "/auth" ? (
				<header style={{ backgroundColor: colors.navigationBackground }} className='w-screen animate-[fadeIn_0.2s_ease] fixed flex gap-4 backdrop-blur-3xl z-10 p-4'>
					<a href='/search' style={{ backgroundColor: url == "/search" ? colors.text : colors.buttonInactiveBg }} className='rounded-full duration-150 p-[0.625rem]'>
					...
					{StatesRender(url)}
					<a href='/notifs' style={{ backgroundColor: url == "/notifs" ? colors.text : colors.buttonInactiveBg }} className='rounded-full duration-150 p-[0.625rem]'>
