...
export default function QuickReactions({ reactionClicked, quickReactions = [], reactions, id }) {
	...
	useEffect(() => {
		document.getElementById(`reactionsScroller-${id}`).addEventListener("touchstart", e => e.stopPropagation());

		return () => {
			document.getElementById(`reactionsScroller-${id}`).removeEventListener("touchstart", e => e.stopPropagation());
		};
	}, []);
	...
	return (
		<div id={`reactionsBlock-${id}`} style={{ color: theme.white, background: theme.semiTransparentBg }} className='rounded-full p-[0.1875rem] gap-3 flex w-[3.125rem] min-w-[3.125rem]'>
			...
			<div id={`reactionsScroller-${id}`} className='w-full overflow-scroll flex gap-4 items-center'>
				{quickReactions.map((reaction, index) => (
